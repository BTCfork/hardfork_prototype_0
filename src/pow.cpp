...
// HFP0 DIF begin: add MIDAS (Multi Interval Difficulty Adjustment System)
// http://dillingers.com/blog/2015/04/21/altcoin-difficulty-adjustment-with-midas/
//
// This is MIDAS, a novel getnextwork algorithm.  It responds quickly to huge changes
// in hashing power, is immune to time warp attacks, and regulates the block rate
// to keep the block height close to the block height expected given the nominal
// block interval and the elapsed time.  How close the correspondence between block
// height and wall clock time is, depends on how stable the hashing power has been.
// Maybe Bitcoin can wait 2 weeks between updates but no altcoin can.

// It is important that none of these intervals (5, 7, 9, 17) have any common divisor;
// eliminating the existence of harmonics is an important part of eliminating the
...
unsigned int GetNextWorkRequiredMIDAS(const CBlockIndex *pindexLast, const CBlockHeader *pblock, const Consensus::Params& params)
{
    ...
    if (params.fPowAllowMinDifficultyBlocks)
    {
        ...
        if (pblock->nTime > pindexLast->nTime + params.nPowTargetSpacing * 2) {
            ...
            while (pindex->pprev && pindex->nBits == nProofOfWorkLimit && pindex->nHeight > params.nHFP0ActivateSizeForkHeight)
                pindex = pindex->pprev;
            // HFP0 DBG begin
...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock, const Consensus::Params& params)
{
    ...
    unsigned int retarget = DIFF_BTC_2016;
    // HFP0 FRK end
...
#if HFP0_DEBUG_DIF
    // HFP0 DBG begin
    if (pindexLast->nHeight + 1 == params.nHFP0ActivateSizeForkHeight) {
        LogPrintf("HFP0 DIF: GetNextWorkRequired detected fork height %d\n", params.nHFP0ActivateSizeForkHeight);
    }
    // HFP0 DBG end
#endif
    if (pindexLast->nHeight + 1 >= params.nHFP0ActivateSizeForkHeight) {
